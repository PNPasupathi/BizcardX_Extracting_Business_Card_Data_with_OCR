...
st.set_page_config(page_title='Bizcart',layout='wide')


# Database Creation
mydb=mysql.connector.connect(host='localhost',user='root',password='3915')
mycurser=mydb.cursor()
mycurser.execute('CREATE DATABASE if not exists bizcard')
mycurser.execute('Use bizcard')
mycurser.execute('Create table if not exists person (Name varchar(50),designation varchar(50),website varchar(50),mail varchar(50),phone varchar(50),street varchar(50),district varchar(50),state varchar(50),pincode varchar(50))')
...
#Header
head1,head2=st.columns([13,0.4])
with head1:
    st.markdown("<h2 style= 'color: #64FFF6;font-size: 34px;'><b>BizCardX : Extracting Business Card Data with OCR</b></h2>", unsafe_allow_html=True)

#Option Menu
st.markdown('##')
st.markdown('##')
opt1,opt2,opt3=st.columns([1,6,1])
with opt2:
    selected=option_menu(menu_title='', options=['Home','Card','Records','Update','Delete','Download'],icons=['house','card-heading','file-earmark','pencil-square','trash','download'],orientation='horizontal'
                ,styles={
                "container": { "background-color": "#080654"},
                "icon": {"color": "orange", "font-size": "18px"},
                "nav-link": {"font-size": "18px", "text-align": "left", "margin":"0px", "--hover-color": "#FF2F83"},
                "nav-link-selected": {"background-color": "#F93D6F"}  })

if selected=='Home':
    st.markdown('#')
    st.markdown(
        "<h2 style= 'color: #64FFF6;font-size: 30px;'>Problem Statement </h2>",
        unsafe_allow_html=True)
    ...
    with h1:
        ...
        st.markdown('''<p style= 'color: white;font-size: 16px;text-align: justify;'>
                ...
                information.</p>''',
                    unsafe_allow_html=True)
        st.markdown(
            "<h2 style= 'color: #64FFF6;font-size: 30px;'>Process</h2>",
            unsafe_allow_html=True)
        st.markdown('''<p style= 'color: white;font-size: 16px;text-align: justify;'>
                        1. Install Packages : Python, OCR, MySQL, Streamlit, Regular Expression, Pandas</br>
                        </br>
                        2. Design the User Interface Using Streamlit</br>
                        </br>
                        3. Extract the Text Data from the Business Card Using OCR</br>
                        </br>
                        4. Store the Extracted Data into MySQL DataBase</br>
                        </br>
                        5. Display the Extracted Information </p>''',
                    unsafe_allow_html=True)
        st.markdown(
            "<h2 style= 'color: #64FFF6;font-size: 30px;'>More Information About OCR</h2>",
            unsafe_allow_html=True)
    ...
    with h3:
        st.image('Images/card1.jpg')
        st.write('#')
        st.image('Images/card.jpg')

                                        #Card
elif selected=='Card':
    try:
        st.markdown(
            "<h2 style= 'color: #64FFF6;font-size: 30px;'>BizCard to Text</h2>",
            unsafe_allow_html=True)
        st.markdown('#')
        ...
        with col3:
                ...
                with c1:
                    ...
                    if st.session_state["Extract"]:
                        ...
                        result=reader.readtext(f'Cards/{uploaded_files[0].name}'.format(uploaded_files[0].name))
                        detlst=[]
    ...
    try:
        st.markdown("<h2 style= 'color: #64FFF6;font-size: 30px;'>DataBase Records</h2>",
                    unsafe_allow_html=True)
        st.markdown('#')
    ...
    try:
        st.markdown('#')
        st.markdown("<h2 style= 'color: #64FFF6;font-size: 30px;'>Update the Record</h2>",
                    unsafe_allow_html=True)
        st.markdown('#')
        namequery = 'select Name from person'
        namelst = []
    ...
    st.markdown('#')
    st.markdown("<h2 style= 'color: #64FFF6;font-size: 30px;'>Delete the Record</h2>",
                unsafe_allow_html=True)
    ...
    st.markdown('#')
    st.markdown("<h2 style= 'color: #64FFF6;font-size: 30px;'>Download the Record</h2>",
                unsafe_allow_html=True)
    namequery = 'select Name from person'
    namelst = []
    mycurser.execute(namequery)
